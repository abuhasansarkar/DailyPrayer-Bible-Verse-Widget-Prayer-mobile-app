...
export const OPENCODE_ZEN_CHAT_ENDPOINT =
...
export type OpenCodeZenModelId =
  (typeof OPENCODE_ZEN_FREE_MODELS)[number]["id"];
...
export async function getOpenCodeZenApiKey(): Promise<string | null> {
  ...
  const envKey =
    process.env.EXPO_PUBLIC_OPENCODE_API_KEY || process.env.OPENCODE_API_KEY;
...
export async function setOpenCodeZenModel(
  modelId: OpenCodeZenModelId,
): Promise<void> {
  await SecureStore.setItemAsync(MODEL_STORAGE_KEY, modelId);
}

export async function askOpenCodeZen(
  prompt: string,
  options?: { system?: string; model?: OpenCodeZenModelId },
): Promise<string> {
...
function generateCompanionFallback(prompt: string, system?: string): string {
  ...
  if (
    ...
    lowerPrompt.includes("pray") ||
    system?.includes("prayer")
  ) {
  ...
  if (
    lowerPrompt.includes("verse") ||
    lowerPrompt.includes("scripture") ||
