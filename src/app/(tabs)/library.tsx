      ...
      <ScrollView contentContainerStyle={{ paddingBottom: 40 }} showsVerticalScrollIndicator={false}>
            ...
            {(['favorites', 'collections', 'history'] as const).map((tab) => (
              ...
              >
                <Text style={{
                  ...
                  {tab === 'favorites' ? t('library.favorites') : tab === 'collections' ? t('library.collections') : t('library.history')}
                </Text>
              </Pressable>
            ))}
          </ScrollView>
        ...
        <Animated.View entering={FadeInDown.duration(400).delay(200)} style={{ paddingHorizontal: 20, gap: 12 }}>
          {activeTab === 'favorites' && (
            favorites.length === 0 ? (
              <View style={{ alignItems: 'center', paddingVertical: 60, gap: 12 }}>
                ...
                <Text style={{ fontFamily: 'Inter_600SemiBold', fontSize: 18, color: textPrimary }}>{t('library.emptyFavorites.title')}</Text>
                <Text style={{ fontFamily: 'Inter_400Regular', fontSize: 15, color: textSecondary, textAlign: 'center', paddingHorizontal: 16 }}>
              ...
              favorites.map((fav) => (
                ...
                >
                  {fav.verse_text && (
                    <>
                      <Text style={{ fontFamily: 'Lora_400Regular_Italic', fontSize: 15, lineHeight: 24, color: textPrimary, marginBottom: 10 }}>
                        "{fav.verse_text.slice(0, 150)}{fav.verse_text.length > 150 ? '...' : ''}"
                      </Text>
          ...
          {activeTab === 'collections' && (
            <View style={{ alignItems: 'center', paddingVertical: 60, gap: 12 }}>
              <Text style={{ fontSize: 56 }}>📚</Text>
              <Text style={{ fontFamily: 'Inter_600SemiBold', fontSize: 18, color: textPrimary }}>No collections yet</Text>
              <Text style={{ fontFamily: 'Inter_400Regular', fontSize: 15, color: textSecondary, textAlign: 'center', paddingHorizontal: 16 }}>
                Create a collection to group your favorite verses and prayers.
              </Text>
              <Pressable style={{ height: 48, paddingHorizontal: 24, borderRadius: 24, backgroundColor: '#F2B84B', alignItems: 'center', justifyContent: 'center' }}>
                <Text style={{ fontFamily: 'Inter_600SemiBold', fontSize: 15, color: '#292B28' }}>{t('library.createCollection')}</Text>
              </Pressable>
            </View>
          )}
          ...
          {activeTab === 'history' && (
            <View style={{ alignItems: 'center', paddingVertical: 60, gap: 12 }}>
              <Text style={{ fontSize: 56 }}>📖</Text>
              <Text style={{ fontFamily: 'Inter_600SemiBold', fontSize: 18, color: textPrimary }}>Reading history</Text>
