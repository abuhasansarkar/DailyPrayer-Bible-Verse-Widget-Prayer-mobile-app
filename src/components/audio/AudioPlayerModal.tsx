...
import { View, Text, Modal, TouchableOpacity, Slider } from 'react-native';
  ...
  return (
    <Modal visible={visible} animationType="slide" transparent>
      <View className="flex-1 bg-black/50 justify-end">
        <View className="bg-[#FFF9EE] rounded-t-3xl p-6 border-t border-[#E8DFC9]">
          {/* Header */}
          <View className="flex-row items-center justify-between mb-6">
            <View className="flex-row items-center">
              ...
              <View>
                <Text className="text-lg font-bold text-[#292B28]">Audio Companion</Text>
                <Text className="text-xs text-[#77766F]">Listen to Scripture & Guided Audio</Text>
              </View>
            </View>
            <TouchableOpacity onPress={handleClose} className="p-2 rounded-full bg-[#F5EDD8]">
          ...
          {verseText && (
            <View className="bg-[#F5EDD8] rounded-2xl p-4 mb-6 border border-[#E8DFC9]">
              ...
              <Text className="text-sm font-semibold text-[#292B28] mb-3" numberOfLines={2}>
              ...
              >
                {isPlayingText ? (
                  <>
                    <Square size={18} color="#292B28" className="mr-2" />
                    <Text className="font-bold text-[#292B28]">Stop Reading</Text>
                  </>
                ) : (
                  <>
                    <Play size={18} color="#292B28" className="mr-2" />
          ...
          {/* Ambient Soundscapes */}
          <Text className="text-xs font-bold text-[#77766F] uppercase tracking-wider mb-3">
              ...
              return (
                <TouchableOpacity
                  ...
                  className={`flex-row items-center p-3 rounded-2xl border ${
                    isSelected ? 'bg-[#FEF3D1] border-[#F2B84B]' : 'bg-[#F5EDD8] border-[#E8DFC9]'
                  }`}
                >
                  <Text className="text-2xl mr-3">{item.icon}</Text>
                  <View className="flex-1">
                    <Text className="font-bold text-[#292B28]">{item.title}</Text>
                    <Text className="text-xs text-[#77766F]">{item.description}</Text>
                  </View>
